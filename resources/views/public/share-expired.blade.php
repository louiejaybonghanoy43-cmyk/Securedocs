    ...
    <div class="container mx-auto px-6 py-8 flex-1 flex items-center justify-center">
        ...
        <div class="bg-[#3C3F58] w-full max-w-lg p-8 mb-4 md:p-12 rounded-2xl text-center">
            ...
            <div class="flex items-center justify-center gap-3 mb-4">
                ...
                <h2 class="text-xl font-bold text-[#f89c00] tracking-wider uppercase">
                    @switch($reason ?? 'default')
                        @case('expired')
                            {{ __('auth.se_title') }}
                            @break
                        @case('used')
                            {{ __('auth.se_title_used') }}
                            @break
                        @case('limit_reached')
                            {{ __('auth.se_title_limit') }}
                            @break
                        @default
                            {{ __('auth.se_title_unavailable') }}
                    @endswitch
                </h2>
            </div>
            ...
            <p class="text-gray-300 text-sm mb-4">
                @switch($reason ?? 'default')
                    @case('expired')
                        ...
                        @break
                    @case('used')
                        {{ __('auth.se_desc_used') }}
                        @break
                    @case('limit_reached')
                        {{ __('auth.se_desc_limit') }}
                        @break
                    @default
                        {{ __('auth.se_desc_unavailable') }}
                @endswitch
            </p>
                ...
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0">
                        @if($share->file->is_folder)
                            <img src="{{ asset('folder.png') }}" alt="Folder Icon" class="w-10 h-10 flex-shrink-0">
                        @else
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-sm text-gray-300">
                            @if($share->expires_at)
                                {{ __('auth.se_expired_on') }} {{ $share->expires_at->format('M d, Y h:i A') }}
                            @else
